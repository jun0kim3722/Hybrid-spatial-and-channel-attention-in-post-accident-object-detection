...
# ------------------- proposed model -------------------
class G_Det(nn.Module):
    def __init__(self, in_channels, out_channels, none):
        super(G_Det, self).__init__()
        self.local_channel_att = LocalChannelAttention(5)
        self.local_spatial_att = SpatialAttention()
        self.global_channel_att = GlobalChannelAttention(5)
        self.fusion_weights = nn.Parameter(torch.Tensor([0.333, 0.333, 0.333])) # equal intial weights

    def forward(self, x):
        global_att = self.global_channel_att(x)

        local_channel_att = self.local_channel_att(x)
        local_att = self.local_spatial_att(local_channel_att)
        local_att = (local_att * x) + x
        ...
        out = (all_feature_maps * weights).sum(1)
...
class ChannelGate(nn.Module):
    ...
    def forward(self, x):
        ...
        avg_att_raw = self.gn( avg_pool )

        max_pool = F.max_pool2d( x, (x.size(2), x.size(3)), stride=(x.size(2), x.size(3)))
        max_att_raw = self.gn( max_pool )

        channel_att_sum = max_att_raw + avg_att_raw
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        ...
        # except:
        #     pdb.set_trace()
        return torch.cat(x, self.d) # ([1, 256, 2, 2])
